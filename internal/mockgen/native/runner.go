...
func NewRunner(manager *runnermanager.Manager, pn, ifs, source, dest, pkg, imp, af, bf, mn, spkg, cf, eo string, po, wpc, dp bool) *Runner {
	var wpcp *bool
	if wpc != true {
...
func (r *Runner) Run() error {
	if err := r.Manager.Run(
		...
		r.ProgOnly,
		r.WritePkgComment,
		r.DebugParser); err != nil {
		return fmt.Errorf("request mockgen run to manager: %w", err)
	}
